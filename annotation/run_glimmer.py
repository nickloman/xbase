...
GLIMMER_PATH = "/home/nick/bio/pkg/glimmer3.02/bin/"
...
cmd = "%s/long-orfs -n -t 1.15 %s %s/ref.longorfs" % (GLIMMER_PATH, concat_reference_file,  working_dir)
print cmd
os.system(cmd)

cmd = "%s/extract -t %s %s/ref.longorfs > %s/ref.train" % (GLIMMER_PATH, concat_reference_file, working_dir, working_dir)
print cmd
os.system(cmd)

cmd = "%s/build-icm -r %s/ref.icm < %s/ref.train" % (GLIMMER_PATH, working_dir, working_dir)
print cmd
os.system(cmd)

cmd = "%s/glimmer3 -o%d -g%d -t30 %s %s/ref.icm %s/seq" % (GLIMMER_PATH, gene_max_overlap, gene_min_length, sequence_file, working_dir, working_dir)
print cmd
os.system(cmd)
